...
export default function DatasetsPage() {
  ...
  async function handleCreate(e: React.FormEvent) {
    e.preventDefault();
    try {
      await datasetsAPI.create({ projectId: id, ...formData });
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {datasets.map((dataset) => (
                <tr key={dataset.id}>
                  ...
                  <td className="px-6 py-4 text-gray-600">{dataset.description || '-'}</td>
                  <td className="px-6 py-4">{dataset.itemCount || 0}</td>
                  <td className="px-6 py-4">
                    <Link
                      href={`/projects/${id}/datasets/${dataset.id}`}
